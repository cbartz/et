...
and then resets that workspace back to a bare "ET-<n>" slot, freeing it for
reuse. Has no Typer/CLI dependency.
"""
...
def complete_task_for_current_workspace(comment: str | None = None) -> TaskCompleteResult:
    ...
    the GNOME workspace is renamed to match, freeing the slot for a future
    `et task create`.
